...
class TestStep:
    ...
    def test_step_pick_drop_full_cycle(self, env):
        ...
        # solo_delivery: robot at (5,5), shelf o1 at (1,1), packing at (8,2)
        # We'll move the robot adjacent to (1,1) and pick
        # Move robot up from (5,5) to (2,1): up 3 then left 4
        # Actually move up to row 2 (3 steps), then left to col 1 (4 steps)
        for _ in range(3):
        ...
        # Robot should now be at (2,1), adjacent to shelf at (1,1)
        assert obs.robots[0].row == 2
        assert obs.robots[0].col == 1
        # Pick
        obs = e.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="pick")]))
        assert obs.robots[0].carrying_item is True

        # Now move to packing station at (8,2)
        # Down from row 2 to row 7: 5 steps down
        for _ in range(6):
            obs = e.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_down")]))
        # Move right from col 1 to col 3: 2 steps
        obs = e.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_right")]))
        # Now at (8,2) ideally — check position is adjacent to (8,2)
        # Drop
        obs = e.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="drop")]))
        # Reward should reflect delivery at some point; check order status
        # At least check that reward is being tracked
    ...
    def test_step_pick_sets_carrying_item(self, env):
        ...
        obs = env.reset(task_id="solo_delivery")
        # Move robot from (5,5) to (2,1) — adjacent to shelf at (1,1)
        for _ in range(3):
            env.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_up")]))
        for _ in range(4):
    ...
    def test_step_drop_at_packing_station_delivers_order(self, env):
        ...
        obs = env.reset(task_id="solo_delivery")
        # Move to (2,1) — adjacent to shelf at (1,1)
        for _ in range(3):
            env.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_up")]))
        for _ in range(4):
            env.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_left")]))
        # Pick
        env.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="pick")]))
        # Move to (7,2) — adjacent to packing at (8,2)
        for _ in range(5):
            env.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_down")]))
        obs = env.step(WarehouseAction(robots=[RobotAction(robot_id=0, action_type="move_right")]))
        # Drop
